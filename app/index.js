...
export const handler = async (event, context, callback) => {
  ...
  let [username] = event.identitySource;
  ...
  if (user) {
    ...
    var userGroups = await getUserGroups(username);
    var hasAdminRole = isAdmin(userGroups);
    payload["role"] = hasAdminRole ? "Admin" : "User";

    var token = generateAccessToken(payload);
    var policy = generateAuthPolicy(username, event.routeArn, hasAdminRole, token)
    console.log(token);
...
const isAdmin = (userGroups) => userGroups.indexOf("admin") >= 0;
